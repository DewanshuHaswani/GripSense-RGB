...
import {
  ...
  createInitialRealSenseRuntime,
  DEFAULT_REALSENSE_ENDPOINT,
  isRealSenseResultFresh,
  REALSENSE_OFFLINE_INTERVAL_MS,
  REALSENSE_REQUEST_INTERVAL_MS,
  refineRealSenseOfflineTimeline,
  requestRealSenseDepthSignal,
  type RealSenseRuntime
} from './vision/realsense';
import { stabilizeV12DisplayAnalysis, type V12DisplayState } from './vision/v12Grip';
import {
...
const YOLO_MAX_BATCH_ENDPOINT = VITE_ENV?.VITE_GRIPSENSE_YOLO_MAX_BATCH_ENDPOINT ?? '/api/gripsense/yolo/offline-max/process';
...
function rfdetrEndpointForVersion(version: AlgorithmVersion) {
  if (version === 'v11' || version === 'v12' || version === 'v13') return YOLO_ENDPOINT;
...
type YoloMaxBatchResponse = {
  ...
  timeline: YoloMaxBatchTrackPoint[];
  artifacts: {
    mp4: string;
    csv: string;
    json: string;
  ...
  filenames?: Partial<Record<'mp4' | 'csv' | 'json' | 'report', string>>;
};
...
const TRAINING_TARGET_VERSIONS: AlgorithmVersion[] = ['v13', 'v12', 'v11', 'v8', 'v10', 'v9', 'v4', 'v5', 'v6'];
...
function trainingTargetInfoForVersion(version: AlgorithmVersion) {
  if (version === 'v13') {
    return {
      version,
      shortLabel: 'V13 YOLO',
      description: 'This profile assists V13 only: YOLO finds the object mask, then the trained profile helps verify the target before V13 applies occlusion-stable object-in-hand scoring. It does not retrain YOLO weights.'
    };
  }
  if (version === 'v12') {
    return {
      version,
      shortLabel: 'V12 YOLO',
      description: 'This profile assists V12 only: YOLO finds the object mask, then the trained profile can verify the object before V12 applies stricter object-in-hand scoring. It does not retrain YOLO weights.'
    };
  }
  if (version === 'v11') {
    return {
  ...
  return {
    version: 'v13' as AlgorithmVersion,
    shortLabel: 'V13 YOLO',
    description: 'Profiles are most useful in V13/V12/V11 YOLO, V8/V10 RF-DETR, V9 RealSense, and V4/V5/V6 trained-object modes. Select one of those targets before training.'
  };
}
...
const EXPLAIN = {
  ...
  objectTrainer: 'Open a separate enrollment portal. Live grip scoring pauses there so you can capture or upload object images without needing the app to believe a grip is already happening. In V13/V12/V11 this creates a local profile assist for YOLO masks; it does not retrain YOLO weights.',
  trainerSteps: 'The guided flow is add object images, review quality suggestions, train a local profile, save it, then enable it. V13/V12/V11 use enabled profiles to verify and name YOLO objects.',
  ...
  objectIdentity: 'How closely the current locked object matches the trained profile. V13/V12/V11 use this as a YOLO profile assist after YOLO finds the mask.',
  trainedProfiles: 'Saved local object profiles. Enabled profiles are used by V4/V5/V6 and as V13/V12/V11 YOLO profile assist; disabled profiles stay saved but are ignored.',
...
export default function App() {
  ...
  const v13DisplayRef = useRef<V13DisplayState>(createInitialV13DisplayState());
  ...
  const updateRealSenseRuntime = useCallback((next: RealSenseRuntime) => {
    realsenseRuntimeRef.current = next;
  ...
  }, [resetRealSenseRuntime, resetRfdetrRuntime, resetV3Runtime]);
  ...
  const processOfflineYoloMaxBatch = useCallback(async (file: File) => {
    ...
    try {
      ...
      form.append('interval', '0.5');
      form.append('threshold', '0.18');
      form.append('maxWidth', '640');
      form.append('outputFps', '30');

      const response = await fetch(YOLO_MAX_BATCH_ENDPOINT, {
        method: 'POST',
        body: form
      });
      if (!response.ok) {
        throw new Error(`YOLO Max batch failed with ${response.status}`);
      }
      const batch = normalizeYoloMaxBatchResponse((await response.json()) as YoloMaxBatchResponse);
      const timeline = mapYoloMaxBatchTimeline(batch.timeline);
      const report = buildOfflineReport(timeline, file.name, batch.metadata.duration ?? video.duration ?? 0);
      offlineYoloMaxBatchRef.current = batch;
      offlineYoloMaxServerPreviewRef.current = true;
      offlineTimelineRef.current = timeline;
  ...
  }, [processOfflineYoloMaxBatch, resetRealSenseRuntime, resetRfdetrRuntime]);
  ...
  const scheduleRfdetrInference = useCallback((video: HTMLVideoElement, hand: Landmark[] | null, timestamp: number, offline = false, provider: DetectorProvider = RFDETR_PROVIDER) => {
    ...
    void requestRfdetrFrameAnalysis({
      ...
      const stillRelevant =
        ...
        algorithmVersionRef.current === 'v12' ||
        algorithmVersionRef.current === 'v13' ||
  ...
  const scheduleRealSenseInference = useCallback((hand: Landmark[] | null, object: ObjectRegion | null, timestamp: number, offline = false) => {
    const current = realsenseRuntimeRef.current;
    const interval = offline ? REALSENSE_OFFLINE_INTERVAL_MS : REALSENSE_REQUEST_INTERVAL_MS;
    if (current.status === 'pending' || timestamp - current.lastRequestAt < interval) return;
    if (!hand || !object) {
      updateRealSenseRuntime({
        ...current,
        status: 'idle',
        message: 'RealSense waiting for hand and object geometry.',
        lastRequestAt: timestamp
      });
      return;
    }
    ...
    updateRealSenseRuntime({
      ...current,
      status: 'pending',
      message: offline ? 'RealSense sampling offline depth evidence.' : 'RealSense sampling live depth evidence.',
      lastRequestAt: timestamp
    });

    const controller = new AbortController();
    const timeout = window.setTimeout(() => controller.abort(), offline ? 1800 : 1200);
    void requestRealSenseDepthSignal({
      hand,
      object,
      ...
      if (result.ok && result.response.available) {
        updateRealSenseRuntime({
          ...latest,
          status: 'ready',
          message: `RealSense depth active (${Math.round(result.response.contactDepthScore * 100)}% contact).`,
          result: result.response,
          receivedAt: result.receivedAt,
          latencyMs: result.response.latencyMs ?? null
        });
        return;
      }
      updateRealSenseRuntime({
        ...latest,
        status: 'unavailable',
        message: result.ok ? 'RealSense depth unavailable; using RGB/RF-DETR evidence only.' : `RealSense unavailable: ${result.status}.`,
        result: null,
        receivedAt: result.receivedAt,
        latencyMs: null
      });
  ...
  const runLoop = useCallback(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      if (!pausedRef.current) {
        ...
        const liveYoloReview =
          (activeAlgorithmVersion === 'v11' || activeAlgorithmVersion === 'v12' || activeAlgorithmVersion === 'v13') && !offlineReview;
        const liveV12Review = activeAlgorithmVersion === 'v12' && !offlineReview;
        const liveV13Review = activeAlgorithmVersion === 'v13' && !offlineReview;
        ...
        if (offlineYoloMaxReview && offlineReportRef.current && offlineTimelineRef.current.length > 0) {
          ...
          if (timelinePoint) {
            ...
            if (!offlineYoloMaxServerPreviewRef.current) {
        ...
        const fallbackAlgorithmVersion: AlgorithmVersion =
          activeAlgorithmVersion === 'v3' || activeAlgorithmVersion === 'v9' ? 'v2' : activeAlgorithmVersion;
            ...
            if (liveRfdetrReview || liveRealSenseReview || liveYoloReview || rfdetrGrip.object) {
              ...
              if (liveYoloReview && rfdetrGrip.object && enabledProfiles.length) {
                ...
                const yoloProfileMatch = matchObjectProfiles(yoloProfileDescriptor, enabledProfiles, {
                  threshold: liveV12Review || liveV13Review ? 0.52 : 0.54,
                  margin: 0.04,
                  useExemplars: true
                });
                if (yoloProfileMatch) {
              ...
              realsenseSelectionMetrics =
                liveRealSenseReview || offlineDepthReview
                  ? {
                      ...
                      stereoConfidence: freshRealSense ? (latestRealSense.result?.stereoConfidence ?? 0) : 0
                    }
                  : null;
            }
          }
        ...
        if (activeAlgorithmVersion === 'v3') {
          ...
          if (liveV13Review) {
            const stabilizedV13 = stabilizeV13DisplayAnalysis(baseFrameAnalysis, v13DisplayRef.current, timestamp, {
          ...
          } else if (liveV12Review) {
            const stabilizedV12 = stabilizeV12DisplayAnalysis(baseFrameAnalysis, v12DisplayRef.current, timestamp, {
              objectScore: rfdetrSelectionMetrics?.objectScore ?? 0,
              contact: rfdetrSelectionMetrics?.contact ?? 0,
              hasObject: rfdetrSelectionMetrics?.hasObject ?? false,
              missedFrames: rfdetrSelectionMetrics?.missedFrames ?? 0
        ...
        if (
          ...
          const point = {
            ...
            realsenseDepthContact: realsenseSelectionMetrics?.depthContact,
            realsenseDepthSeparation: realsenseSelectionMetrics?.depthSeparation,
    ...
    if (animationRef.current) cancelAnimationFrame(animationRef.current);
    animationRef.current = requestAnimationFrame(tick);
  }, [analysis, mediaMode, scheduleRealSenseInference, scheduleRfdetrInference, scheduleV3Inference, updateCalibrationCapture]);
  ...
  const selectAlgorithmVersion = useCallback(
    (version: AlgorithmVersion) => {
      ...
      v13DisplayRef.current = createInitialV13DisplayState();
      targetProfileIdRef.current = null;
      targetBaseIdRef.current = null;
      stabilizerRef.current.reset();
      resetV3Runtime(
        version === 'v3'
          ...
          : version === 'v12'
          ? 'V12 selected. Start the local YOLO server; V12 adds stricter object-in-hand validation before scoring.'
          : version === 'v13'
          ? 'V13 selected. Start the local YOLO server; V13 adds occlusion recovery and visual-lock smoothing on top of V12.'
          : 'V3 server idle. Select V3 and start tracking to begin fusion.'
      );
      resetRfdetrRuntime(
        version === 'v11' || version === 'v12' || version === 'v13'
          ? `${version.toUpperCase()} selected. Start tracking to connect to the local YOLO server.`
      ...
      setAnalysis(
        createEmptyAnalysis(
          version === 'v3'
            ...
            : version === 'v12'
            ? 'V12 selected. It uses YOLO masks plus stricter object-in-hand scoring so empty hands cannot keep a grip label.'
            : version === 'v13'
            ? 'V13 selected. It keeps V12 object-in-hand safety but bridges short finger occlusions and smooths the visible lock.'
            : version === 'v2'
    ...
    const payload =
      format === 'json'
        ...
        : [
            ...
            'time,grip,confidence,objectMatch,lock,contact,closure,thumb,enclosure,slip,weak,guidance,object,mode,state,objectX,objectY,objectRadiusX,objectRadiusY,objectAngle,palmX,palmY,rfdetrObjectScore,rfdetrContact,rfdetrLatencyMs,realsenseDepthContact,realsenseDepthSeparation,realsenseStereoConfidence',
            ...offlineTimelineRef.current.map((point) =>
              [
                ...
                point.realsenseDepthContact === undefined ? '' : Math.round(point.realsenseDepthContact * 100),
                point.realsenseDepthSeparation === undefined ? '' : Math.round(point.realsenseDepthSeparation * 100),
                point.realsenseStereoConfidence === undefined ? '' : Math.round(point.realsenseStereoConfidence * 100)
              ].join(',')
            )
          ].join('\n');
  ...
  return (
    ...
    >
          ...
          <div className="toolbar" aria-label="Camera controls">
            <label className="version-select" aria-label="Algorithm version">
              ...
              >
                ...
                <option value="v12">V12 · YOLO production</option>
                <option value="v13">V13 · YOLO stable live</option>
              </select>
            </label>
          ...
          <div className="offline-review-overlay" aria-label="Offline grip video review">
            <div className="offline-glass-panel offline-left">
              ...
              <div className="offline-version-toggle" aria-label="Offline review algorithm version">
                ...
                <button
                  type="button"
                  className={offlineReviewVersion === 'v3' ? 'active' : ''}
                  onClick={() => switchOfflineReviewVersion('v3')}
                >
                  V3
                </button>
                <button
            ...
            <div className="offline-glass-panel offline-right">
              ...
              {isOfflineDepthVersion(offlineReviewVersion) && (
                <>
                  ...
                  <GlassMetric label="Stereo confidence" value={offlineTimeline[offlineTimeline.length - 1]?.realsenseStereoConfidence ?? 0} />
                </>
              )}
            ...
            <div className="portal-grid">
              ...
              <div className="portal-side">
                ...
                <label className="training-target-field">
                  ...
                  >
                    <option value="v13">V13 YOLO stable assist</option>
                    <option value="v12">V12 YOLO production assist</option>
      ...
      {mediaMode === 'live' && <aside className="analysis-rail" aria-label="Grip analysis">
        ...
        {(algorithmVersion === 'v8' || algorithmVersion === 'v9' || algorithmVersion === 'v10' || algorithmVersion === 'v11' || algorithmVersion === 'v12' || algorithmVersion === 'v13') && (
          <div className="v3-panel">
            <div className="motion-header">
              <Activity size={18} />
              <span>{algorithmVersion === 'v9' ? 'V9 RealSense' : algorithmVersion === 'v10' ? 'V10 RF-DETR proxy' : algorithmVersion === 'v13' ? 'V13 YOLO stable' : algorithmVersion === 'v12' ? 'V12 YOLO production' : algorithmVersion === 'v11' ? 'V11 YOLO' : 'V8 RF-DETR'}</span>
            </div>
            <div className={rfdetrRuntime.status === 'ready' ? 'v3-status ready' : 'v3-status fallback'}>
              <span>{algorithmVersion === 'v9' ? `RF-DETR ${formatRfdetrRuntimeStatus(rfdetrRuntime)}` : algorithmVersion === 'v11' || algorithmVersion === 'v12' || algorithmVersion === 'v13' ? `YOLO ${formatRfdetrRuntimeStatus(rfdetrRuntime)}` : formatRfdetrRuntimeStatus(rfdetrRuntime)}</span>
            ...
            {algorithmVersion === 'v9' && (
              <>
                <div className={realsenseRuntime.status === 'ready' ? 'v3-status ready' : 'v3-status fallback'}>
                  <span>RealSense {formatRealSenseRuntimeStatus(realsenseRuntime)}</span>
                  <strong>{realsenseRuntime.status === 'ready' ? `${Math.round((realsenseRuntime.result?.contactDepthScore ?? 0) * 100)}% depth` : '--'}</strong>
                </div>
                <p className={realsenseRuntime.status === 'ready' ? 'diagnostic-copy' : 'diagnostic-copy warn'}>{realsenseRuntime.message}</p>
              </>
            )}
            <div className="v3-score-grid">
              <V3Score label={algorithmVersion === 'v9' ? 'RGB object' : algorithmVersion === 'v11' || algorithmVersion === 'v12' || algorithmVersion === 'v13' ? 'YOLO object' : 'RF object'} value={objectDetection?.score ?? 0} />
            ...
            {algorithmVersion === 'v9' && (
              <div className="v3-score-grid">
                <V3Score label="Depth gap" value={realsenseRuntime.result?.depthSeparationScore ?? 1} />
                <V3Score label="Stereo" value={realsenseRuntime.result?.stereoConfidence ?? 0} />
                <V3Score label="Surface" value={realsenseRuntime.result?.surfaceContinuity ?? 0} />
                <V3Score label="Occlusion" value={realsenseRuntime.result?.occlusionScore ?? 0} />
              </div>
            )}
            <div className="diagnostic-row neutral">
              <span>Latency</span>
              <strong>
                {algorithmVersion === 'v9'
                  ? `${rfdetrRuntime.latencyMs === null ? '--' : Math.round(rfdetrRuntime.latencyMs)} / ${realsenseRuntime.latencyMs === null ? '--' : Math.round(realsenseRuntime.latencyMs)} ms`
                  : rfdetrRuntime.latencyMs === null
                  ? '--'
                  : `${Math.round(rfdetrRuntime.latencyMs)} ms`}
              </strong>
            </div>
          </div>
        )}
        ...
        <div className="trainer-panel">
          ...
          <div className="training-target-summary">
            ...
            <strong>{trainingTargetInfo.version === 'v13' ? 'V13 only' : trainingTargetInfo.version === 'v12' ? 'V12 only' : trainingTargetInfo.version === 'v11' ? 'V11 only' : trainingTargetInfo.version.toUpperCase()}</strong>
          </div>
          <p className="diagnostic-copy">{trainingStatus}</p>
...
function normalizeYoloMaxBatchResponse(response: YoloMaxBatchResponse): YoloMaxBatchResponse {
  return {
    ...response,
    artifacts: {
      mp4: normalizeYoloMaxArtifactUrl(response.artifacts.mp4),
      csv: normalizeYoloMaxArtifactUrl(response.artifacts.csv),
      json: normalizeYoloMaxArtifactUrl(response.artifacts.json),
      report: normalizeYoloMaxArtifactUrl(response.artifacts.report)
    }
  };
}

function normalizeYoloMaxArtifactUrl(url: string) {
  if (!url.startsWith('/api/yolo/offline-max/artifact')) return url;
  const batchPath = new URL(YOLO_MAX_BATCH_ENDPOINT, window.location.origin).pathname;
  return batchPath.startsWith('/api/gripsense/yolo/')
    ? url.replace('/api/yolo/offline-max/artifact', '/api/gripsense/yolo/offline-max/artifact')
    : url;
}

function mapYoloMaxBatchTimeline(points: YoloMaxBatchTrackPoint[]): OfflineTimelinePoint[] {
  return points.map((point) => {
    const objectScore = clampNumber(point.object_score ?? point.score ?? 0, 0, 1);
    const contact = clampNumber(point.contact ?? 0, 0, 1);
    const grip = clampNumber(point.grip ?? 0, 0, 1);
    const width = point.width ?? 0;
    const height = point.height ?? 0;
    const hasObjectGeometry = point.x !== null && point.y !== null && width > 0 && height > 0;
    const centerX = hasObjectGeometry ? (point.x ?? 0) + width / 2 : null;
    const centerY = hasObjectGeometry ? (point.y ?? 0) + height / 2 : null;
    const state = grip >= 0.68 ? 'strong-hold' : grip >= 0.34 ? 'grip-detected' : objectScore >= 0.2 ? 'object-uncertain' : 'no-hand';
    const guidance = grip >= 0.68 ? 'Strong grip' : grip >= 0.34 ? 'Improve grip' : objectScore >= 0.2 ? 'Object not locked' : 'Object not locked';

    return {
      time: point.time,
      grip: Math.round(grip * 100),
      confidence: Math.max(grip, contact * 0.9, objectScore * 0.55),
      objectMatch: objectScore,
      lock: objectScore,
      contact,
      closure: contact,
      thumb: contact > 0.2 ? clampNumber(contact * 0.72, 0, 1) : 0,
      enclosure: contact,
      slip: clampNumber(point.slip ?? 0, 0, 1),
      weak: Boolean(point.weak),
      guidance,
      object: '',
      mode: grip >= 0.68 ? 'Power Grip' : grip >= 0.34 ? 'Uncertain' : 'Uncertain',
      state,
      objectX: centerX,
      objectY: centerY,
      objectRadiusX: hasObjectGeometry ? width / 2 : null,
      objectRadiusY: hasObjectGeometry ? height / 2 : null,
      objectAngle: 0,
      palmX: null,
      palmY: null,
      rfdetrObjectScore: objectScore,
      rfdetrContact: contact,
      rfdetrLatencyMs: point.latency_ms
    };
  });
}
...
function formatRealSenseRuntimeStatus(runtime: RealSenseRuntime) {
  if (runtime.status === 'ready') return 'ready';
  if (runtime.status === 'pending') return 'pending';
  if (runtime.status === 'unavailable') return 'unavailable';
  return 'idle';
...
function stabilizeV11DisplayAnalysis(
  ...
  if (lockCollapsed) {
    ...
    const canBridgeDetectorBlink =
      ...
      isV11StillGripPose(analysis) &&
      !isV11LikelyObjectDropped(analysis) &&
      lossAge <= 520;
    ...
    const decay = clampUnit(lossAge / 520);
    const fallbackGrip = Math.round(Math.max(24, Math.min(68, previous.gripPercentage * (1 - decay * 0.58))));
    const objectLockQuality = clampUnit(Math.max(0.28, Math.min(0.54, previous.objectLockQuality * (1 - decay * 0.52))));
    const confidence = clampUnit(Math.max(0.26, Math.min(0.56, previous.confidence * (1 - decay * 0.46))));
    const guidance: GripAnalysis['guidance'] = fallbackGrip >= 44 ? 'Improve grip' : 'Reposition';
    const stabilized: GripAnalysis = {
      ...analysis,
      gripPercentage: fallbackGrip,
      confidence,
      objectLockQuality,
      closureScore: Math.max(analysis.closureScore, previous.closureScore * (1 - decay * 0.5)),
      thumbOpposition: Math.max(analysis.thumbOpposition, previous.thumbOpposition * (1 - decay * 0.5)),
      enclosureScore: Math.max(analysis.enclosureScore, previous.enclosureScore * (1 - decay * 0.5)),
      motionCoupling: Math.max(analysis.motionCoupling, previous.motionCoupling * (1 - decay * 0.54)),
      slipRisk: Math.max(analysis.slipRisk, Math.min(0.46, previous.slipRisk + decay * 0.22)),
      guidance,
      message: 'YOLO lost the object for a moment; holding a degraded grip estimate while checking if it returns.',
      evidence: {
        ...analysis.evidence,
        objectLockQuality,
        temporalLockScore: Math.max(analysis.evidence.temporalLockScore, previous.evidence.temporalLockScore * (1 - decay * 0.6)),
        visibleContactScore: Math.max(analysis.evidence.visibleContactScore, previous.evidence.visibleContactScore * (1 - decay * 0.56)),
        fingerSegmentContactScore: Math.max(analysis.evidence.fingerSegmentContactScore, previous.evidence.fingerSegmentContactScore * (1 - decay * 0.56))
      },
      diagnostics: {
        ...analysis.diagnostics,
        state: fallbackGrip >= 44 ? 'Grip detected' : 'Object uncertain',
        recommendation: 'Hold steady; if the object is still present the YOLO lock should recover.',
        issueCategory: 'object_uncertain',
        objectIssue: 'YOLO object evidence blinked; using short temporal hold before declaring the object lost.'
      }
    };

    return { analysis: stabilized, state: { analysis: stabilized, timestamp, softLossStartedAt } };
  }
  ...
  return { analysis: stabilized, state: { analysis: stabilized, timestamp, softLossStartedAt: null } };
}
...
function isV11StillGripPose(analysis: GripAnalysis) {
  const openHandScore = analysis.evidence.modeScores['open hand'] ?? 0;
  const wrapSignal = Math.max(
    analysis.closureScore,
    analysis.enclosureScore,
    analysis.evidence.fingerCurlScore,
    analysis.evidence.phoneSideGripScore,
    analysis.evidence.powerGripScore,
    analysis.evidence.palmObjectContainmentScore
  );
  return analysis.diagnostics.state !== 'No hand' && openHandScore < 0.74 && wrapSignal >= 0.36;
}

function isV11LikelyObjectDropped(analysis: GripAnalysis) {
  return (
    analysis.slipRisk >= 0.58 ||
    analysis.motionState === 'slipping' ||
    analysis.diagnostics.issueCategory === 'motion_problem' ||
    analysis.diagnostics.state === 'No hand' ||
    (analysis.diagnostics.state === 'Hand only' &&
      analysis.closureScore < 0.32 &&
      analysis.evidence.fingerCurlScore < 0.38)
  );
}
...
function readInitialAlgorithmVersion(): AlgorithmVersion {
  ...
  if (fromUrl === 'v1' || fromUrl === 'v2' || fromUrl === 'v3' || fromUrl === 'v4' || fromUrl === 'v5' || fromUrl === 'v6' || fromUrl === 'v7' || fromUrl === 'v8' || fromUrl === 'v9' || fromUrl === 'v10' || fromUrl === 'v11' || fromUrl === 'v12' || fromUrl === 'v13') return fromUrl;
  const fromStorage = window.localStorage.getItem(ALGORITHM_VERSION_STORAGE_KEY);
  return fromStorage === 'v1' || fromStorage === 'v2' || fromStorage === 'v3' || fromStorage === 'v4' || fromStorage === 'v5' || fromStorage === 'v6' || fromStorage === 'v7' || fromStorage === 'v8' || fromStorage === 'v9' || fromStorage === 'v10' || fromStorage === 'v11' || fromStorage === 'v12' || fromStorage === 'v13'
    ? fromStorage
    : 'v5';
}
